...
function fetchedImages(value) {
  ...
  return fetch(`${BASE_URL}?key=${API_KEY}&q=${value}&image_type=photo&page=${pages}&per_page=12&orientation=horizontal&safesearch=true`)
    .then(res => { return res.json() })
    .then(img => {

      console.log(`hi, thera are ${img.totalHits}`)
